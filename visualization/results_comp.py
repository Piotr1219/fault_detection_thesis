...
if __name__ == '__main__':
    ...
    #     rects = []
    #     print(res[1][i])
    #     rects.append(ax.bar(offset, res[1][i].astype(float), width,
    #                label='Detection rate', color='tab:orange'))
    #     offset += 6
    # # ax.bar(res[0, :], res[1, :], width=0.2)
    # methods = res[0, :]
    # print(methods)
    #
    # ax.set_xticks(x + width*2.5*(x+1) - 5, methods)
    # ax.set_axisbelow(True)
    # ax.yaxis.grid(color='gray')
    # plt.ylim(0.8, 1.02)
    ...
    # plt.savefig('models_3c_mean_f1.eps')
    # plt.show()
    ...
    for file in sorted(os.listdir('./download_1')):
        if 'Clustering_test.csv' in file:
            ...
            for model in models:
                ...
                for err, color in zip(errors, colors):
                    print(file, err)
                    sc += (df[df['model_pret']==model][df['Errors_types'] == err]['Detection_rate'].max())
                    multiplier += 1
                    c += 1
    ...
    # plt.ylabel('F1 score')
    plt.xticks(rotation=18)
    plt.savefig('models_3c_mean_clustering_detection_rate.eps')
    plt.show()
